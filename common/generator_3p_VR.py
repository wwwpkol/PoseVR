...
class ChunkedGenerator:
    def __init__(self, batch_size, cameras, poses_3d, poses_2d, poses_vr,
        ...
        #####VR
        self.batch_vr = np.empty((batch_size, chunk_length + 2 * pad, 3, 3))
    ...
    def get_batch(self, seq_i, start_3d, end_3d, flip, reverse):
        ...
        #vr
        start_vr = start_3d - self.pad - self.causal_shift
        end_vr = end_3d + self.pad - self.causal_shift
        ...
        if self.poses_3d is not None:
            ...
            # vr
            if self.poses_vr is not None:
                seq_vr = self.poses_vr[seq_name].copy()
                # seq_vr = self.poses_vr[seq_i]
                low_vr = max(start_vr, 0)
                high_vr = min(end_vr, seq_vr.shape[0])
                pad_left_vr = low_vr - start_vr
                pad_right_vr = end_vr - high_vr
                if pad_left_vr != 0 or pad_right_vr != 0:
                    self.batch_vr = np.pad(seq_vr[low_vr:high_vr],
                                              ((pad_left_vr, pad_right_vr), (0, 0), (0, 0)), 'edge')
                else:
                    self.batch_vr = seq_vr[low_vr:high_vr]

                if flip:

                    self.batch_vr[:, [1,2]] = self.batch_vr[:, [2,1]]
                if reverse:
                    self.batch_vr = self.batch_vr[::-1].copy()
        ...
        if self.poses_3d is None and self.cameras is None:
            ...
            return self.batch_cam, self.batch_3d.copy(), self.batch_2d.copy(), self.batch_vr.copy(), action, subject, int(cam_index)
